...
| [`flta_eval/fl.py`](../flta_eval/fl.py) | **Tier A** — pure-numpy FedAvg + DP-SGD + RDP accountant (tutorial-scale demo path) |
| [`flta_eval/fl_torch.py`](../flta_eval/fl_torch.py) | **Tier B** — PyTorch CNN (TinyCNN, 25k params) + DP-SGD FedAvg + MPS acceleration (Position B SOTA-faithful baseline) |
| [`flta_eval/attacks.py`](../flta_eval/attacks.py) | FL-native attacks: gradient inversion (Geiping); per-record MIA against released model with three meta-classifiers (logistic / LiRA [Carlini 2022] / RMIA-online [Zarifzadeh 2024]); bootstrap CIs + per-class stratification; shadow-target FL parity via `build_shadow_pool` + `per_record_mia_pool` |
| [`flta_eval/audit.py`](../flta_eval/audit.py) | Audit-trail discipline + empirical-ε lower-bound utility (Jagielski 2020) |
...
| [`scripts/compare_accountants.py`](../scripts/compare_accountants.py) | RDP-envelope vs PRV-accountant comparison on the chain's DP configuration |
| [`scripts/multi_seed_sq5.py`](../scripts/multi_seed_sq5.py) | Multi-seed SQ-5 ordering-stability runner; outputs an aggregate record under `results/sq5/` |
