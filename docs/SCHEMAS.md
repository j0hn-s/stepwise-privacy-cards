...
**Note on Tier-B SOTA result records.** Tier B introduces an additional result-record shape (`attack: "mia.per_record.sota"`, `variant: "SQ-1d"`) under [`../results/sq1/`](../results/sq1/) — config carries `model_class: "TinyCNN-25k"`, `device`, `n_shadow_models`, `pod_fraction`; result carries `{lira, rmia}.{worst_record_tpr, ci_lower, ci_upper, median_tpr, per_class}` plus a `riskcal_002_fires_lira` / `_rmia` and `ci_binds_lira` / `_rmia` Boolean pair surfacing the meta-classifier verdict + CI tightness. The card's `evidence_ref` may point at either Tier A or Tier B records (or both, as separate `evidence_ref` entries) — the rule engine's RISKCAL-002 reads `measured.target_advantage` and is agnostic to which tier produced it.

### 3.2 SOLID-* per-subject rules

| Rule ID | Severity | Fires when |
|---|---|---|
| SOLID-CONSENT-MISSING | RED | Consent receipt absent from the pod |
| SOLID-CONSENT-EXPIRED | RED | Consent receipt `not_after` is in the past relative to evaluation time |
| SOLID-CONSENT-CONTROLLER | RED | Consent receipt `controller` ≠ deployment controller URI declared on the card |
| SOLID-CONSENT-SIGNATURE | AMBER | Consent receipt signature does not verify against the subject's WebID (mocked) |
...
The three consent rules were split out of an earlier `SOLID-CONSENT-001` because a single rule ID conflated three semantically distinct findings — a reviewer reading a `SOLID-CONSENT-001` finding could not tell which of the three triggered. `SOLID-CONSENT-SIGNATURE` is the renamed `SOLID-CONSENT-002` for naming consistency.
