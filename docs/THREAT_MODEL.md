...
**Scope statement (read first).** The evaluation is calibrated against an **honest-but-curious server** — a coordinator that follows the FL protocol but logs and analyses everything it sees. *Adversarial-server attacks* in which the coordinator crafts gradients to exfiltrate inputs [Boenisch *et al.*, USENIX Security 2023, §5] are out of scope, and the COMP-GRADINV-001 ε* = 4.0 threshold is conservative for the honest-but-curious profile only. Deployments in which the coordinator cannot be assumed honest-but-curious — e.g., where MPC secure aggregation is required and the coordinator is treated as fully adversarial — require a tighter threshold and a different calibration regime; this is named explicitly because conflating the two profiles is the most common over-reading of FL privacy claims [Boenisch 2023, §1].
